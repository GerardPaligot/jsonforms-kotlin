...
/**
 * Get the label of a field from the [Property] or the [Control] with '*' if it is required.
 * If there is no title in the [Property] or label in the [Control], it'll return the id as label.
 *
 * ```kotlin
 * val property = StringProperty()
 * val control = Control(scope = "#/properties/$id")
 * val label = property.label(required = true, control = control)
 * ```
 *
 * @param required Field is required or not
 * @param control Field contained in the [com.decathlon.jsonforms.models.uischeme.UiSchema].
 * @return label with an '*' if the field is required
 */
fun Property.labelWithId(required: Boolean, control: Control): String {
    val label = label(required, control)
    if (label != null) return label
    val suffix = if (required) "*" else ""
    val id = control.propertyKey()
    return "$id$suffix"
}
 ...
 * ```
 *
 * @param valueLength Length of the current value, or 0 if value is null
 * @param control Field contained in the [com.decathlon.jsonforms.models.uischeme.UiSchema].
